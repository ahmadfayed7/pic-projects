...
const unsigned short TEMP_RESOLUTION = 12;                                      // Choose Resolution for DS 18B20 here
...
void Display_Temperature(unsigned int temp2write) {
  ...
  neg = 0;
   ...
   if ( Chip == 0x10 ) temp2write = temp2write * 8;
      temp=temp&0b0000011111110000;
      temp=temp>>4;
      IntToStr(temp,txt2);
      lcd_out(2,1,txt2) ;
  // check if temperature is negative

if (temp2write & 0xf000)
     {
      text[0] = '-';
  ...
  // extract temp_fraction and convert it to unsigned int
   ...
   temp_fraction = (float)temp_fraction *0.0625 ;//25/4;
